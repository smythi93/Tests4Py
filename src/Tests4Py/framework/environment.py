...
def __env_on__(project: Project, verbose=True) -> Environment:
    ...
    environ['PATH'] = f'{Path(os.environ["PYENV_ROOT"], "versions", v, "bin")}:{os.environ["PATH"]}'
    LOGGER.info(f'Check for activated python version')
    output = subprocess.check_output(['python', '--version'], env=environ).decode('utf-8').replace('\n', '')
...
def __activating_venv__(work_dir: Path, environ: Environment, verbose=True) -> Environment:
    if verbose:
        LOGGER.setLevel(logging.INFO)
    else:
        LOGGER.setLevel(logging.WARNING)
