...
class ToGrammarVisitor(NodeVisitor):
    ...
    def visit_Module(self, node: Module) -> str:
        return f'Module({self.visit_list(node.body)},' \
    ...
    def visit_FunctionType(self, node: FunctionType) -> str:
        return f'FunctionType({self.visit_list(node.argtypes)},' \
    ...
    def visit_AsyncFor(self, node: AsyncFor) -> str:
        return f'AsyncFor({self.visit(node.target)},' \
               f'{self.visit(node.iter)},' \
               f'{self.visit_list(node.body)},' \
               f'{self.visit_list(node.orelse)},' \
               f'{"" if node.type_comment is None else self.visit(node.type_comment)})'
    ...
    def visit_With(self, node: With) -> str:
        return f'With({self.visit_list(node.items)},' \
               f'{self.visit_list(node.body)},' \
               f'{"" if node.type_comment is None else self.visit(node.type_comment)})'

    def visit_AsyncWith(self, node: AsyncWith) -> str:
        return f'AsyncWith({self.visit_list(node.items)},' \
               f'{self.visit_list(node.body)},' \
               f'{"" if node.type_comment is None else self.visit(node.type_comment)})'

    def visit_Match(self, node: Match) -> str:
        return f'Match({self.visit(node.subject)},' \
               f'{self.visit_list(node.cases)})'

    def visit_Raise(self, node: Raise) -> str:
        return f'Raise({"" if node.exc is None else self.visit(node.exc)},' \
               f'{"" if node.cause is None else self.visit(node.cause)})'

    def visit_Try(self, node: Try) -> str:
        return f'Try({self.visit_list(node.body)},' \
    ...
    def visit_ImportFrom(self, node: ImportFrom) -> str:
        return f'ImportFrom({"" if node.module is None else node.module},' \
    ...
    def visit_BoolOp(self, node: BoolOp) -> str:
        return f'BoolOp({self.visit(node.op)},' \
               f'{self.visit_list(node.values)})'

    def visit_NamedExpr(self, node: NamedExpr) -> str:
        return f'NamedExpr({self.visit(node.target)},' \
               f'{self.visit(node.value)})'

    def visit_BinOp(self, node: BinOp) -> str:
        return f'BinOp({self.visit(node.left)},' \
               f'{self.visit(node.op)},' \
               f'{self.visit(node.right)})'

    def visit_UnaryOp(self, node: UnaryOp) -> str:
        return f'UnaryOp({self.visit(node.op)},' \
               f'{self.visit(node.operand)})'
    ...
    def visit_IfExp(self, node: IfExp) -> str:
        return f'IfExp({self.visit(node.test)},' \
               f'{self.visit(node.body)},' \
               f'{self.visit(node.orelse)})'

    def visit_Dict(self, node: Dict) -> str:
        return f'Dict({self.visit_list(node.keys)},' \
               f'{self.visit_list(node.values)})'
    ...
    def visit_Attribute(self, node: Attribute) -> str:
        return f'Attribute({self.visit(node.value)},' \
               f'{node.attr})'

    def visit_Subscript(self, node: Subscript) -> str:
        return f'Subscript({self.visit(node.value)},' \
               f'{self.visit(node.slice)})'

    def visit_Starred(self, node: Starred) -> str:
        return f'Starred({self.visit(node.value)},' \
               f'{self.visit(node.ctx)})'
    ...
    def visit_Slice(self, node: Slice) -> str:
        return f'Slice({"" if node.lower is None else self.visit(node.lower)},' \
    ...
    def visit_comprehension(self, node: comprehension) -> str:
        return f'comprehension({self.visit(node.target)},' \
               f'{self.visit(node.iter)},' \
               f'{self.visit_list(node.ifs)},' \
               f'{node.is_async})'

    def visit_ExceptHandler(self, node: ExceptHandler) -> str:
        return f'ExceptHandler({"" if node.type is None else self.visit(node.type)},' \
               f'{"" if node.name is None else node.name},' \
               f'{self.visit_list(node.body)})'

    def visit_arguments(self, node: arguments) -> str:
        return f'arguments({self.visit_list(node.posonlyargs)},' \
               f'{self.visit_list(node.args)},' \
    ...
    def visit_MatchMapping(self, node: MatchMapping) -> str:
        return f'MatchMapping({self.visit_list(node.keys)},' \
    ...
    def visit_MatchClass(self, node: MatchClass) -> str:
        return f'Await({self.visit(node.cls)},' \
               f'{self.visit_list(node.patterns)},' \
    ...
    def visit_MatchAs(self, node: MatchAs) -> str:
        return f'MatchAs({"" if node.pattern is None else self.visit(node.pattern)},' \
    ...
    def visit_TypeIgnore(node: TypeIgnore) -> str:
        return f'TypeIgnore({node.lineno},' \
               f'"{node.tag}")'
...
grammar: Grammar = {
    ...
    '<While>': [
        'While(<expr>,<stmt_list>,<stmt_list>)'
    ],
    '<If>': [
        'If(<expr>,<stmt_list>,<stmt_list>)'
    ],
    '<With>': [
    ...
    '<Match>': [
        'Match(<expr>,<match_case_list>)'
    ],
    '<Raise>': [
        'Raise(<optional_expr>,<optional_expr>)',
    ],
    '<Try>': [
        'Try(<stmt_list>,<excepthandler_list>,<stmt_list>,<stmt_list>)'
    ],
    # '<TryStar>': [
    ...
    '<Dict>': [
        'Dict(<expr_list>,<expr_list>)'
    ],
    '<Set>': [
        'Set(<expr_list>)'
    ],
    '<ListComp>': [
        'ListComp(<expr>,<comprehension_list>)'
    ],
    '<SetComp>': [
        'SetComp(<expr>,<comprehension_list>)'
    ],
    '<DictComp>': [
        'DictComp(<expr>,<expr>,<comprehension_list>)'
    ],
    '<GeneratorExp>': [
        'GeneratorExp(<expr>,<comprehension_list>)'
    ],
    '<Await>': [
        'Await(<expr>)'
    ],
    ...
    '<Compare>': [
        'Compare(<expr>,<cmpop_list>,<expr_list>)'
    ],
    '<Call>': [
        'Call(<expr>,<expr_list>,<keyword_list>)'
    ],
    '<FormattedValue>': [
        'FormattedValue(<expr>,<int>,<optional_expr>)'
    ],
    '<JoinedStr>': [
        'JoinedStr(<expr_list>)'
    ],
    ...
    '<comprehension>': [
        '<comprehension>(<expr>,<expr>,<expr_list>,<int>)'
    ],
    ...
    '<MatchClass>': ['MatchClass(<expr>,<pattern_list>,<identifier_list>,<pattern_list>)'],
    '<MatchStar>': [
